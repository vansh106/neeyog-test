...
export default function QuotationFinancialEditor({ quotation, disabled = false, onSaved }: Props) {
  const [draft, setDraft] = useState<QuotationFinancialDraft>(() => hydrateFinancialDraft(quotation))
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [dirty, setDirty] = useState(false)

  useEffect(() => {
    setDraft(hydrateFinancialDraft(quotation))
    setDirty(false)
    setError(null)
  }, [quotation])

  const preview = useMemo(
    () => computeFinancialPreview(quotation.subtotal, draft),
    [quotation.subtotal, draft],
  )
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      const updated = await quotationsApi.updateFinancialSummary<Quotation>(
        ...
        <div className="rounded-lg border border-[#E2E6DC] p-4">
          ...
          <table className="w-full border-collapse text-[13px]">
            <tbody>
              ...
              {!draft.cgstApplicable && !draft.sgstApplicable && !draft.igstApplicable && preview.gstAmount > 0 && (
                <tr className="border border-[#333] border-t-0 bg-white">
                  <td className="px-2 py-1.5 font-bold">GST</td>
                  <td className="px-2 py-1.5 text-right font-mono">{formatCurrency(preview.gstAmount)}</td>
                </tr>
              )}
