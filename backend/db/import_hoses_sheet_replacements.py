"""Replace all six Hoses catalog sheets from the replacement workbook and load supplier prices.

Clears each ``catalog_fp_hose_*`` table, re-imports rows from
``docs/Final_Products/replacement/Hoses_Products (1).xlsx``, then upserts
``supplier_product_prices`` for supplier Tuder (first sheet) and Jyoti Sales (other five).

Usage (from ``backend/``):
  python -m db.import_hoses_sheet_replacements --dry-run
  python -m db.import_hoses_sheet_replacements
"""
...
from db.hose_sheet_constants import (  # noqa: E402
    HOSE_CATALOG_KEYS,
    HOSE_REPLACEMENT_FILENAME,
    HOSE_REPLACEMENT_SHEETS,
)
...
REPLACEMENT_PATH = DOCS_FINAL / "replacement" / HOSE_REPLACEMENT_FILENAME
...
async def _run(dry_run: bool) -> None:
    ...
    if dry_run:
        ...
        logger.info("Dry run — would replace hoses catalog + prices for client_id=%s", client_id)
        for sheet_name, catalog_key, supplier_name in HOSE_REPLACEMENT_SHEETS:
            ...
            for data_row in ws.iter_rows(min_row=2, values_only=True):
                ...
                if price_ix is not None and price_ix < len(data_row) and _cell_float(data_row[price_ix]) is not None:
                    priced += 1
            logger.info(
                "  %s -> %s supplier=%s rows=%d priced=%d headers=%s",
                sheet_name,
                catalog_key,
                supplier_name,
                n,
                priced,
                sorted(idx_map.values()),
            )
        wb.close()
        return
    ...
    async with async_session_factory() as session:
        ...
        for _, _, supplier_name in HOSE_REPLACEMENT_SHEETS:
        ...
        hose_models = [MODEL_BY_KEY[k] for k in HOSE_CATALOG_KEYS if k in MODEL_BY_KEY]
        for model in hose_models:
            await session.execute(delete(model).where(model.client_id == client_id))
        await session.execute(
            delete(SupplierProductPrice).where(
                SupplierProductPrice.catalog_table.in_(sorted(HOSE_CATALOG_KEYS)),
            )
        )
        await session.flush()
        ...
        for sheet_name, catalog_key, supplier_name in HOSE_REPLACEMENT_SHEETS:
            ...
            for data_row in rows_iter:
                ...
                kwargs: dict[str, Any] = {
                    "row_id": uuid.uuid4(),
                    "client_id": client_id,
                    "source_file": HOSE_REPLACEMENT_FILENAME,
                }
                empty = True
                for col_idx, field in idx_map.items():
                    if col_idx >= len(data_row):
        ...
        logger.info(
            "Hose prices: created=%d updated=%d unmatched=%d no_price=%d",
            price_stats["created"],
            price_stats["updated"],
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="Replace all Hoses masters sheets from replacement workbook")
