"""Re-import FBV catalog from ``Flush_Bottom_Valve_Products - F (1).xlsx`` and load Casco list prices.

Clears only ``catalog_fp_fbv_*`` rows for ACTIVE_CLIENT, re-imports both worksheets,
removes prior Casco supplier prices for those categories, then upserts prices from the Price column.

Usage (from ``backend/``):
  python -m db.import_fbv_catalog_and_prices --dry-run
  python -m db.import_fbv_catalog_and_prices
  python -m db.import_fbv_catalog_and_prices --supplier-name "Casco"
"""
...
FBV_FILENAME = "Flush_Bottom_Valve_Products - F (1).xlsx"

FBV_SHEETS: list[tuple[str, str, type]] = [
    ("FBV – Ball Type", "fp_fbv_ball_type", fpm.CatalogFpFbvBallTypeRow),
    ("FBV – Y Type", "fp_fbv_y_type", fpm.CatalogFpFbvYTypeRow),
]

FBV_CATALOG_KEYS = [k for _, k, _ in FBV_SHEETS]
...
async def _run(dry_run: bool, supplier_name: str) -> None:
    ...
    path = DOCS_FINAL / FBV_FILENAME
    if not path.is_file():
        raise FileNotFoundError(f"Missing workbook: {path}")

    if dry_run:
        wb = load_workbook(path, read_only=True, data_only=True)
        logger.info("Dry run — would re-import FBV catalog + prices for client_id=%s", client_id)
        for sheet_name, catalog_key, model in FBV_SHEETS:
    ...
    async with async_session_factory() as session:
        ...
        fbv_models = [m for _, _, m in FBV_SHEETS]
        for model in fbv_models:
        ...
        await session.execute(
            delete(SupplierProductPrice).where(
                ...
                SupplierProductPrice.catalog_table.in_(FBV_CATALOG_KEYS),
            )
        )
        await session.flush()
        ...
        logger.info(
            "Casco FBV prices: created=%d updated=%d unmatched=%d no_price=%d",
            price_stats["created"],
            price_stats["updated"],
        ...
        for catalog_key, model in [(k, m) for _, k, m in FBV_SHEETS]:
            cat_n = (
                await session.execute(
...
def main() -> None:
    ...
    p.add_argument("--supplier-name", default="Casco", help="Supplier name match (default: Casco)")
    args = p.parse_args()
